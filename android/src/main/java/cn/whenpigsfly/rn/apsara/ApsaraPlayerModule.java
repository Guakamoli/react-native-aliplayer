...
public class ApsaraPlayerModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void setGlobalSettings(final ReadableMap options, final Promise promise) {
        String localCacheDir = getAliVideoPreloadDir(mReactContext);
        try {
            if (options != null && options.toHashMap().size() > 0) {
                int maxBufferMemoryKB = options.hasKey("maxBufferMemoryKB") ? options.getInt("maxBufferMemoryKB") : 1024 * 10;
                AliPlayerGlobalSettings.enableLocalCache(true, maxBufferMemoryKB, localCacheDir);
                int expireMin = options.hasKey("expireMin") ? options.getInt("expireMin") : 24 * 60 * 2;
                int maxCapacityMB = options.hasKey("maxCapacityMB") ? options.getInt("maxCapacityMB") : 1024 * 5;
                int freeStorageMB = options.hasKey("freeStorageMB") ? options.getInt("freeStorageMB") : 1024 * 5;
